...
- (NSDictionary *)toMap;
@end
@interface FLTPrepareFiltersMessage ()
...
- (NSDictionary *)toMap {
  return [NSDictionary dictionaryWithObjectsAndKeys:(self.textureId ? self.textureId : [NSNull null]), @"textureId", (self.filterId ? self.filterId : [NSNull null]), @"filterId", nil];
}
@end
...
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    ...
    case 134:     
      return [FLTPrepareFiltersMessage fromMap:[self readValue]];
...
- (void)writeValue:(id)value 
{
  ...
  if ([value isKindOfClass:[FLTPrepareFiltersMessage class]]) {
    [self writeByte:134];
    [self writeValue:[value toMap]];
  } else 
...
void FLTAVFoundationVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.useFilter"
        binaryMessenger:binaryMessenger
        codec:FLTAVFoundationVideoPlayerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(useFilter:error:)], @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(useFilter:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTFilterMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api useFilter:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.prepareFilters"
        binaryMessenger:binaryMessenger
        codec:FLTAVFoundationVideoPlayerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(prepareFilters:error:)], @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(prepareFilters:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTPrepareFiltersMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api prepareFilters:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
